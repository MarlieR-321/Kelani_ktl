...
class UpDeCategoriaFragment : Fragment () {
    ...
    private fun GuardarCambios() {
        ...
        {
            //Crear el objeto
            val categoria = Categoria(args.currentCategoria.id_categoria, nomb,desc,2)
            //Actualizar
            viewModel.actualizarCategoria(categoria)
            Toast.makeText(requireContext(), "Registro actualizado",
                Toast.LENGTH_LONG).show()
            findNavController().navigate(R.id.action_upDeCategoriaFragment_to_nav_category)
        }
        else
        {
    ...
    private fun EliminarCategoria() {
        ...
        alerta.setPositiveButton("Si") {_, _ ->
            ...
            //Crear el objeto
            val categoria = Categoria(args.currentCategoria.id_categoria, nomb,desc,3)
            //Eliminar
